...
所有的运算元都不是原始值，所以`+`从最左边开始触发数值转换。`Object`和`Array`的`valueOf`方法都返回自身，所以被忽略了。`toString()`作为备选被使用。这里的一个技巧是，第一个`{}`不是当做一个对象的词法，而是作为一个块声明，所以被忽略了。接着计算下一个`+[]`表达式，先是通过`toString()`方法转换为空字符串，然后再转换为`0`。
