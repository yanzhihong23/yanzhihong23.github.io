 ...
 * @LastEditors: Hom Yan
 * @LastEditTime: 2019-06-11 19:44:21
 -->
...
这些方法不是一个 JavaScript 规范的一部分。但大多数环境都有内部调度器，并提供这些方法。特别是，浏览器和 Node.js 都支持。
...
对`setTimeout`的一次调用会返回一个“计时器标识符” `timeId`，我们可以用来取消执行。
...
alert(timerId); // 计时器标识符

clearTimeout(timerId);
alert(timerId); // 同样的标识符（不会因为取消而变成null）
```

从`alert`的输出结果我们可以看出，在浏览器中计时器标识符就是一个数字。在其他环境中，有可能是其他的。例如，Node.js 返回的是一个带有常用方法的计时器对象。

再一次，这些方法没有统一的规范，所以没关系。

对于浏览器，计时器在 HTML5 标准的[计时器章节](https://www.w3.org/TR/html5/webappapis.html#timers)有描述。
...
> **`alert`显示的时候计时不会停下来**
>
> 大多数浏览器，包括 Chrome 和 Firefox，在显示`alert/confirm/prompt`时，内部的计时器都还在继续“嘀嗒”。
>
...
如果我们调度的函数是很消耗 CPU 的，那么我们计算执行所花费的时间，并计划更早或更晚执行下一次调用。
...
为了更清晰点，我们来考虑一个更简单的例子。假如我们有一个函数，需要从`1`计数到`1000000000`。
...
值得注意的是，两种变体 - 无论是否通过 setTimeout 分割作业 - 都具有可比性。 总计数时间没有太大差异。
...
> **浏览器内嵌计时器的最小延迟**
>
> 在浏览器中，有一个内嵌计时器运行频率的限制。HTML5 标准提到：“在 5 个嵌套计时器之后，间隔被强制为至少 4 毫秒。”
>
